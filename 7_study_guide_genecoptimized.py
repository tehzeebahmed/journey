...
Key improvements:
- Reduced demo API calls from 6 to 1, how? By implementing response caching. 
If the same topic is requested again, the cached result is returned instead of making 
a new API call. This can save up to 80% of API calls for repeated topics.
- Response caching to prevent duplicate requests (from line 200 onwards) 
for example, if a user generates a study guide for "Python basics" and then tries to 
generate it again, the second request will hit the cache and return the stored result 
without making an API call.
- Rate limiting to safely stay within free tier limits, How? By tracking the timestamps of 
API requests and pausing execution if the number of requests in the 
last minute approaches the defined RPM limit 
(default: 30). This prevents "quota exceeded" errors while allowing for a 
reasonable number of requests. from line 250 onwards.
...
from typing import Optional, Dict, List # For type hints, Optional for parameters that can be None, Dict and List for structured data types
from pathlib import Path # For handling file paths in a cross-platform way
from datetime import datetime, timedelta # For tracking request timestamps for rate limiting
from dotenv import load_dotenv
from langchain_google_genai import ChatGoogleGenerativeAI 
# Importing the ChatGoogleGenerativeAI class from the langchain_google_genai module, which is used to interact with Google's Gemini LLM.
from langchain_core.prompts import PromptTemplate, ChatPromptTemplate
# Importing PromptTemplate and ChatPromptTemplate from langchain_core.prompts, 
# which are used to create structured prompts for the LLM.
from langchain_core.output_parsers import StrOutputParser
# Importing StrOutputParser from langchain_core.output_parsers, which is used to parse the output from the LLM into a string format.
from langchain_core.runnables import RunnablePassthrough, RunnableMap
# Importing RunnablePassthrough and RunnableMap from langchain_core.runnables, which are used to create runnable chains for processing data through the LLM. 
# and they are used to create chains that can be invoked with input data to generate outputs, allowing for flexible composition of prompts and LLM calls. in line 300 onwards.
...
# here timestamp, log level, and message are included in the log output for better traceability.
logger = logging.getLogger(__name__)
# Create a logger named after the current module, which can be used to log messages throughout the code.
#__name__ is a special variable in Python that holds the name of the current module. By using it to create a logger, we ensure that the logger is appropriately named based on where it's used, which can help with debugging and log organization.
...
class StudyGuideGenerator:
    # by defining class in a python program, we are creating a blueprint for objects 
    # that can generate study guides based on input topics.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # this function __init__ is the constructor for the StudyGuideGenerator class. 
        # It initializes the LLM, sets up caching and rate limiting configurations, 
        # and prepares prompt templates for generating study guides.
        """
        ...
        self.config = kwargs
        #self.config is a dictionary that stores additional configuration options passed as keyword arguments (kwargs) when creating an instance of the StudyGuideGenerator class. 
        # This allows for flexible configuration of the generator, such as specifying the tone of the study
        # guide (formal or casual) or other parameters that may be needed in the future without changing the class definition.
        ...
        if self.cache_enabled:
            ...
            logger.info(f"\nCache enabled at: {self.cache_dir}")
        
        # RATE LIMITING SETUP
        self.rpm_limit = rpm_limit
        self.request_times: List[datetime] = []
        logger.info(f"\nRate limiting enabled: {self.rpm_limit} RPM")
        
        self._setup_prompts()
        logger.info(f"{self.__class__.__name__} initialized")
    
    def _setup_prompts(self) -> None:
        #what does self does inside _setup_prompts is that it allows us to define instance variables 
        # (like self.prompt_formal and self.prompt_casual) that can be accessed throughout 
        # the class.
    ...
    def _get_cached_result(self, input_text: str, tone: str = "formal") -> Optional[str]:
        ...
        # what this cache will have is that it will store the generated study guide for a given input topic and tone. 
        # When a request is made, it will first check if there is a cached result for that specific input and tone combination. If a cached result exists, it will return it immediately, saving
        # an API call. If not, it will proceed to generate a new study guide and then save it to the cache for future requests.
        ...
        if cache_file.exists():
            try:
                with open(cache_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    logger.info(f"\n Cache HIT (saved 1 API call): '{input_text[:30]}...'")
    ...
    def _check_rate_limit(self) -> None:
        ...
        if len(self.request_times) >= self.rpm_limit:
            ...
            logger.warning(
                f"\n\nRPM limit ({self.rpm_limit}) approaching. "
...
def main():
    ...
    try:
        ...
        # and this tone can be set by the user when creating the StudyGuideGenerator instance, allowing for dynamic configuration of the tone of the generated study guides. in line 300 onwards.
