...
# ── LLM factory ──────────────────────────────────────────────────────────────
def create_mistral_llm() -> Mistral:
    load_dotenv()                                       # FIX 2: load .env first
    logger.info("Reading MISTRAL_API_KEY from environment...")
    api_key = os.getenv("MISTRAL_API_KEY")
    if not api_key:
        msg = "MISTRAL_API_KEY not found. Set it in your .env or environment."
        logger.error(msg)
        raise ValueError(msg)

    logger.info("MISTRAL_API_KEY found. Creating Mistral LLM...")
    # FIX 3: mistral-large-latest has a higher free-tier quota than mistral-medium.
    # Free tier limits (as of 2025): mistral-large-latest = 1500 req/day; mistral-medium = 20 req/day.
    # Swap to "mistral-medium" if you still hit quota limits.
