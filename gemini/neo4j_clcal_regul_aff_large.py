...
from audit_logger import AuditLogger, summarize_results
...
JSONL_FILE_NAME = CURR_PATH.joinpath("synthetic_trial_data.jsonl")
URI = os.getenv("NEO4J_URI")
AUTH = (os.getenv("NEO4J_USERNAME"), os.getenv("NEO4J_KEY"))
neo4j_database = os.getenv("NEO4J_DATABASE")

audit = AuditLogger(CURR_PATH.joinpath("clinical_copilot_audit.jsonl"))
...
def agent_seed_clinical_data(driver, db_name):
    ...
    # with open (JSONL_FILE_NAME, "r") as file_read:
    ...
    // 1. Create Study node
    MERGE (s:Study {study_id: row.study_params.id})
    ON CREATE SET 
                s.title = row.study_params.title, 
                s.phase = row.study_params.phase
    
    // 2. Create Visit node and link via HAS_VISIT
    MERGE (v:Visit {visit_id: row.visit_params.id})
    ON CREATE SET 
                v.name = row.visit_params.name, 
                v.week = row.visit_params.week
    MERGE (s)-[:HAS_VISIT]->(v)
    
    // 3. Create DataElement node and link via COLLECTS
    MERGE (d:DataElement {element_id: row.element_params.id})
    ON CREATE SET 
                d.label = row.element_params.label, 
                d.cdisc_variable = row.element_params.variable
    MERGE (v)-[:COLLECTS]->(d)
    
    // 4. Create ControlledTerm node and link via USES_TERM
    MERGE (c:ControlledTerm {code: row.term_params.code})
    ON CREATE SET 
                c.name = row.term_params.name, 
                c.system = row.term_params.system
    MERGE (d)-[:USES_TERM]->(c)
    
    // 5. Create Subject node and link via HAS_OBSERVATION
    MERGE (sub:Subject {
        usubjid: row.subject.usubjid
    })

    MERGE (sub)-[obs:HAS_OBSERVATION]->(d)
    SET obs.value = row.subject.value,
        obs.unit = row.subject.unit,
        obs.visit_id = row.visit_params.id
    
    // 6. Wrap the single treatment dict object into an array list [ ] for UNWIND
    WITH s, v, d, sub, obs, [row.treatment_params] AS treatments_list, row
    ...
    // 7. Create distinct Observation Node anchors
    MERGE (o:Observation {observation_id: row.observation_params.id})
    ON CREATE SET 
                o.value = row.observation_params.value, 
                o.unit = row.observation_params.unit

    // 8. Connect the unified observational matrix links
    MERGE (sub)-[:MADE_OBSERVATION]->(o)
    MERGE (o)-[:RECORDED_AT]->(v)
    MERGE (o)-[:MEASURED_ELEMENT]-(d)
    
    RETURN count(o) AS total_processed
    """
    # processed_counts = 0
    BATCH_SIZE = 1000
    current_batch = []
    total_processed = 0
    logging.info(f"Starting optimized batch UNWIND ingestion from {JSONL_FILE_NAME}...")

    # Open the JSONL file and parse one line each
    with open(JSONL_FILE_NAME, "r", encoding = "utf-8") as read_file:
        for linenum, line in enumerate(read_file, 1):
            clean_line = line.strip()
            if not clean_line:
                continue
            try:
              # Load exactly ONE line string at a time
              ...
              current_batch.append(formatted_row)
            except Exception as e:
                logging.error(f"Skipping malformed row at line {linenum}: {e}")
                continue
            # if the batch hits the counts flush it to neo4j
            if len(current_batch) >= BATCH_SIZE:
                driver.execute_query(cypher_ingest_batch, {"batch": current_batch}, database_= db_name)
                total_processed += len(current_batch)
                logging.info(f"Processed batch unwind. Total committed: {total_processed} records.")
                current_batch = []
        # flush any remaining data after 1000 rows
        if current_batch:
            driver.execute_query(cypher_ingest_batch, {"batch": current_batch}, database_= db_name)
            total_processed += len(current_batch)
            logging.info(f"Flushed final leftover batch. Complete! Total committed: {total_processed} records.")
        # --------------lets comments this code for large dataset--------------
    ...
    logging.info(f"Ingestion Finished! Total records streamed: {total_processed}")
    return total_processed

def agent_verify_and_read_graph(driver, db_name):
...
- treatment_comparison: agent wants to compare baseline characteristics between two treatments 
...
def agent_generate_cypher(question: str, intent: str) -> tuple[str, dict]:
...
def run_clinical_copilot(driver, db_name):
    ...
    while True:
        ...
        cypher, parameters  = agent_generate_cypher(question, intent)
        if cypher is None:
          print(f"\nThe question cannot be answered from current study data.\n")
          continue

        query_key = "Unknown Query"
        for k, v in QUERY_LIBRARY.items():
            if v["cypher"]==cypher:
                query_key = k
                break
        ...
        t0 = time.time()
        try:
            # Explicitly verify driver is not None right before executing
            if driver is None:
                print("Error: Driver object unexpectedly became None.")
                continue
            results, _, _ = driver.execute_query(cypher, parameters, database_= db_name)
            raw_result = [dict(r) for r in results]
            result_summary = summarize_results(raw_result)
        except Exception as e:
            print(f"error raised at execution - {e}\n")
            audit.log(
                question = question, classified_intent = intent, query_key = query_key,
                cypher = cypher, parameters = parameters, result_summary = result_summary,
                narrative = None, latency_ms = round((time.time() - t0) * 1000, 1),
            )
            continue
        ...
        audit.log(
            question = question, classified_intent = intent, query_key = query_key,
            cypher = cypher, parameters = parameters, result_summary = result_summary,
            narrative = narrative, model = os.getenv("GEMINI_MODEL_NAME"), latency_ms = round((time.time() - t0) *1000 , 1),  
        )
...
def main():
    ...
    # check_validity_of_files(JSONL_FILE_NAME)
    print("Executing seeding of data.....")
    driver = get_neo4j_driver()
    # --- FIX: Stop the execution immediately if the driver is None ---
    if driver is None:
        print("\n[CRITICAL ERROR] Could not establish a connection to Neo4j AuraDB.")
