...
and agents response is vlaidated using pydantic also added adding episodic memory for not validating pan or property for a given time, 
keeping all records in json file
"""
...
MEMORY_FILE_PATH = CURR_PATH.joinpath("a8_loan_memory.json")
...
print (f"The memory json file is - {MEMORY_FILE_PATH}")
...
def load_memory() -> dict:
    """Load memory from JSON file. Return empty dict if file doesn't exist."""
    # YOUR CODE — 4 lines
    # if file exists → open and return json.load()
    try:
        ...
        print(f"Unable to parse the json file {MEMORY_FILE_PATH}")
        return {}

def save_memory(memory: dict):
    """saves memory as dict to json file"""
    try:
        with open(MEMORY_FILE_PATH, "w") as file:
            json.dump(memory, file, indent=4)
    except Exception as e:
        print(f"Error in saving memory")

def get_cahed_results(pan: str, memory: dict) -> dict | None:
    """
        Look up PAN in memory.
        Return cached result if valid, None if expired or not found.
        Also deletes expired records.
        """
    if pan not in memory:
        print(f" [memory] Pan {pan} not found in the cache")
        return None

    record = memory[pan]
    valid_until = datetime.fromisoformat(record["valid_until"])
    if datetime.now() > valid_until:
        print(f"cached record of PAN {pan} expired..deleteing and will be resaved..")
        del memory[pan]
        save_memory(memory)
        return None
    days_left = (valid_until - datetime.now()).days
    print(f"[Memory]- PAN {pan} found in cache and is valid for {days_left}")
    return record

def store_agents_result(pan:str, credit_results: dict, fraud_results: dict, property_results: dict, memory: dict):
    """Store credit agent, fraud agent and property agent's resuls in the momory for 90 days"""
    get_credit_agent_value = credit_results
    get_fraud_agent_value = fraud_results
    get_property_agents_result = property_results
    memory[pan] = {
        "pan_number": pan,
        "credit_score": credit_results["credit_score"],
        "fraud_flag": fraud_results["fraud_flag"],
        "property_value": property_results["property_value"],
        "cached_at": datetime.now().isoformat(),
        "valid_until": (datetime.now() + timedelta(days=CREDIT_VALIDITY_DAYS)).isoformat()
    }
    with open (MEMORY_FILE_PATH, "w") as json_data:
        json.dump(memory, json_data, indent=4)
    return None    
...
def fraud_agent(state: LoanState) -> dict:
    ...
    pan = state["pan_number"]
    profile = PAN_LOOKUP_DB.get(pan)
    if profile is None:
        fraud_flag = False
    else:
        fraud_flag = profile.get("fraud_flag", False)

    response = fraudResponse(
        pan_number= pan, fraud_flag= fraud_flag, checked_at= datetime.now().isoformat()
    )
    # return {"fraud_flag": fraud_flag} 
    print(f"\nFraud agent validated.....")
    return response.model_dump()
...
def property_agent(state: LoanState) -> dict:
    ...
    if profile is None:
        ...
        property_address = profile.get("property_address", "N/A")
    response = propertyResponse(
        property_address=property_address, property_value= property_value, checked_at=datetime.now().isoformat()
    )
    # return  {"property_value": property_value}    # return dict to update state
    print(f"\nProperty agent validated.....")
    return response.model_dump()
...
def supervisor(state: LoanState)-> LoanState:
    ...
    memory = load_memory()
    cached_results = get_cahed_results(state["pan_number"], memory)
    ...
    if cached_results:
        print("Supervisor using the cached results - skipping the agents workload")
        state["credit_score"] = cached_results["credit_score"]
        state["fraud_flag"] = cached_results["fraud_flag"]
        state["property_value"] = cached_results["property_value"]
    else:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            ...
            credit_result   = credit_future.result() # raw dict from agent
            fraud_result    = fraud_future.result()
            property_result = property_future.result()

        # Step 3 — validate through Pydantic models 
        credit_validated = creditResponse(**credit_result)
        fraud_validated = fraudResponse(**fraud_result)
        property_validated = propertyResponse(**property_result)
        ...
        state["fraud_flag"]     = fraud_validated.fraud_flag
        state["property_value"] = property_validated.property_value
        # update state with worker results
    ...
    if int(state["credit_score"]) < 650:
        ...
        state["decision"]= "Rejected"
        state["reason"] = "Frad Detected for PAN"
    elif ltv > 80:
        state["decision"]= "Rejected"
...
def main():
    ...
    if not profile:
        print(f"[MAIN SECTION] - PAN {pan_number} does not exists")
        return
    
    state1 = {
        "applicant_name":   profile.get("applicant_name", "unknown"),
